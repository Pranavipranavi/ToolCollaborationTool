...
export default function KanbanColumn({ column, members, canMoveTask = () => true, onOpenTask }) {
  return (
        ...
        {(provided, snapshot) => (
          ...
          >
            {column.tasks.map((task, index) => (
              <Draggable key={task.id} draggableId={task.id} index={index} isDragDisabled={!canMoveTask(task)}>
                {(dragProvided) => (
                  <TaskCard
